...
void EnemyHealthBar::Draw(sf::RenderWindow& window)
{
	if (visible)
	{
		window.draw(*Sprite);
...
EnemyHealthBar::EnemyHealthBar(std::shared_ptr<GameMediator> gameMediatorPtr, std::string newName, std::string barAssetDirectory, float posX, float posY, float scaleX, float scaleY) : font("JdmgtR34raz9.ttf"), text(font, "")
{
...
void EnemyHealthBar::setText(std::string newText)
{
	text.setString(newText);
}

std::string EnemyHealthBar::getName()
{
	return name;
}

Response EnemyHealthBar::requestData(const std::string& request)
{
	Response hi = "hello";
	return hi;
}

void EnemyHealthBar::notify(const std::string& message)
{
	if (message == "setVisible")
	{
		...
		{
			visible = true;
			Response newDisplayedAttackPoints = gameMediator->requestData("Enemy", "Health");
